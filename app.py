...
st.set_page_config(page_title="SIKI - Final Audit", layout="wide")
st.title("🌊 SIKI (Sistem Manajemen Aset Irigasi)")
st.markdown("✅ **Status:** Enterprise Ready (Valuasi NAB + Rumus Prioritas)")
...
with st.sidebar.expander("🛠️ Admin"):
    if st.button("⚠️ RESET"): app.hapus_semua_data(); st.rerun()
    st.download_button("⬇️ Backup", app.export_ke_json(), "backup.json")
    up = st.file_uploader("⬆️ Restore")
    if up and st.button("Restore"): app.import_dari_json(up); st.rerun()

menu = st.sidebar.radio("Navigasi", ["Dashboard", "1. Master Aset (Statis)", "2. Inspeksi (Dinamis)", "3. Non-Fisik", "4. Laporan & Blangko"])
...
# --- 4. LAPORAN (BLANGKO STYLE) ---
elif menu == "4. Laporan & Blangko":
    ...
    if st.button("Generate Excel Blangko"):
        ...
        with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
            ...
            fmt_currency = wb.add_format({'num_format': 'Rp #,##0', 'border': 1})
            ...
            # Header Custom
            headers = ['Nama Aset', 'Jenis', 'Kondisi Sipil', 'Kondisi ME', 'Skor Bahaya', 'Kelas Prioritas', 'Biaya Rehab', 'NAB']
            for col, h in enumerate(headers):
                ws1.write(0, col, h, fmt_header)
            
            # Isi Data
            if not df_p.empty:
                for row_idx, row in enumerate(df_p.itertuples(), 1):
                    ws1.write(row_idx, 0, row.nama_aset, fmt_cell)
                    ws1.write(row_idx, 1, row.jenis_aset, fmt_cell)
                    ws1.write(row_idx, 2, f"{row.kondisi_sipil}%", fmt_cell)
                    ws1.write(row_idx, 3, f"{row.kondisi_me}%", fmt_cell)
                    ws1.write(row_idx, 4, f"{row.Skor_Prioritas:.1f}", fmt_cell)
                    ws1.write(row_idx, 5, row.Kelas_Prioritas, fmt_cell)
                    ws1.write(row_idx, 6, row.estimasi_biaya, fmt_currency)
                    ws1.write(row_idx, 7, row.nilai_aset_baru, fmt_currency)
